...
cassette 自 0.3.0 起有两个格式版本：

| 版本 | 引入 | 关键差异 |
|---|---|---|
| `1` | 0.3.0 及更早 | 仅非流式记录（`Response` 无 `stream`） |
| `2` | 0.4.0 | 引入 `Response.stream`（流式记录）；追加流式记录后写端自动升到 `2` |

读端向后兼容 **1..2**，所以**绝大多数旧文件无需任何操作就能继续读取**。只有当：

- 你想把旧版本文件**显式升级**为当前版本（例如提交一份统一格式的录制），或
- 你有一份手写 / 跨工具产生的旧格式 cassette，想确认它在新读端下语义不变，
...
# 迁移后自检：通过即表示语义未变（两个摘要都对齐）
moon run cmd/main -- verify new.cassette.json
```

`migrate` 的规则：

- **绝不原地覆盖**——来源与目标是同一文件会直接报错退出（CI 以此守卫）；
- 旧版本读入后若追加了流式记录，输出版本自动升到 `2`；
- `verify` 同时校验请求指纹摘要与记录摘要，谁手工改过文件这里就拦住。

### 跨版本往返

「旧文件 → 新读端 → 回写」不丢信息：读入 `version 1` 的文件，用当前写端重新写出，
字段逐字节稳定（见「测试与验证」里的形状矩阵）。这正是 0.9.0 红队复跑的断言之一。
...
> 自 0.9.0 起，下表三项被锁定为格式契约：**读端向后兼容全部已发布版本；写端恒定输出当前值；升大版本（1.0）前不会变更**。

| 维度 | 当前值 | 读端保证 | 写端保证 | 变更条件 |
|---|---|---|---|---|
| `cassette_format_version` | `2` | 接受 `1..2`（所有历史版本可直接解码，无需迁移） | 恒定写 `2` | 仅 1.0 大版本 |
| `stream_format_version` | `2` | 含流式记录的旧文件可解码 | 追加流式记录时自动升级到 `2` | 仅 1.0 大版本 |
| `content_encoding` | `"base64"` 信封 | 接受 `{"content_encoding":"base64","payload":"<规范 base64>"}` 与普通 JSON 对象（无信封） | 仅产出 base64 信封 | 新增编码方式时 |

历史：`version 1` 为 0.3.0 及更早格式；`version 2` 引入 `Response.stream`（流式记录）。
读取端遇到更高版本会明确报 `UnsupportedVersion`，而不是悄悄丢弃不认识的字段——
后者会让完整性校验以「摘要对不上」的形式失败，用户根本看不出真正原因。
CI 以「**读端必须接受全部历史版本**」为硬校验（不变量 3）。
